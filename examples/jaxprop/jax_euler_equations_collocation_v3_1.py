...
# The maximum mach number is properly computed. Now we are extending the problem to impose the criterion nside
...
def build_residual_vector(Dx, x):
    ...
    def residual(z, parameters):
        ...
        R_u = (Dx @ u)    - N_all[:, 0] / Dn / sD
        R_d = (Dx @ ln_d) - N_all[:, 1] / Dn / (sD * jnp.maximum(d, 1e-12))
        R_p = (Dx @ ln_p) - N_all[:, 2] / Dn / (sD * jnp.maximum(p, 1e-12))

        # Evaluate residual at the boundary conditions
        # x_star, Ma_max = maximum_from_poly_nodes(x, out["Ma"])
        x_star, Ma_max = soft_maximum_from_poly_nodes(x, out["Ma"], alpha=200, oversample=10)
        R_u = R_u.at[0].set(Ma_target - Ma_max) / 1000  # Is it fair to reduce this one?
        R_d = R_d.at[0].set(jnp.log(d0_in/out["d0"][0]))
        R_p = R_p.at[0].set(jnp.log(p0_in/out["p0"][0]))

        # Print current BC residual values (runtime)
        jax.debug.print(
            "Iter: Ma[0]={:.6f}, Ma_max={:.6f}, x_max={:0.6f}, d0[0]={:.6f}, p0[0]={:.6f}",
            out["Ma"][0],
            Ma_max,
            x_star,
            out["d0"][0],
...
# @eqx.filter_jit
def soft_maximum_from_poly_nodes(x_nodes, y_nodes, alpha=100.0, oversample=10):
    """
    Smooth approximation of max over [x1,x2] for the degree-N polynomial interpolant
    through (x_nodes, y_nodes). Returns (x_star, y_star) where y_star is a softmax-
    weighted value and x_star is the soft-argmax location. Differentiable & JIT-safe.
    """
    # MAP x -> t in [-1, 1]
    ...
    # FIT monomial coeffs via Vandermonde (ascending), then switch to descending
    powers = jnp.arange(N + 1)
    V = t_nodes[:, None] ** powers[None, :]
    c_asc = jnp.linalg.solve(V, y_nodes)
    c_desc = c_asc[::-1]

    # EVALUATE on a fixed grid in t
    K = oversample * (N + 1) + 1
    t_grid = jnp.linspace(-1.0, 1.0, K)
    y_grid = jnp.polyval(c_desc, t_grid)

    # SOFTMAX weights; larger alpha → closer to hard max
    w = jax.nn.softmax(alpha * y_grid)
    t_star = jnp.sum(w * t_grid)
    y_star = jnp.sum(w * y_grid)
...
if __name__ == "__main__":
    ...
    Ma_array = jnp.asarray([0.5, 0.8, 0.9, 0.95, 0.95, 0.99, 1.00])
    # Ma_array = jnp.asarray([0.1, 0.101, 0.1002, 0.101, 0.103])
    colors = plt.cm.magma(jnp.linspace(0.2, 0.8, len(Ma_array)))
    z0 = initialize_flowfield(num_points, params, fluid)
    print(type(z0), type(params))
    results = []
    for Ma, color in zip(Ma_array, colors):
        t0 = perf_counter()
        params = tree_map(jnp.asarray, {**params, "Ma_target": Ma})
        ...
        # print(type(z0), type(params_current))
