...
def solve_nozzle_model(
    ...
    ode_args_fixed = (eps_wall, T_wall, wall_friction, heat_transfer, fluid)
    ...
    x_hat, D_hat = chebyshev_lobatto(num_points, -1, +1)
    L = params["x_star0"]
    x = 0.5 * L * (1.0 + x_hat)
    ...
    if initial_guess is None:
        ...
        z0 = jnp.concatenate([z0, jnp.asarray([params["x_star0"]])])
    else:
        z0 = initial_guess
    ...
    solution = opx.least_squares(
        ...
        solver,
        z0,
        args=(ode_args_fixed, inlet_bc, Ma_target),
    ...
    ode_args = (x_star,) + ode_args_fixed
...
def build_residual_vector(D_hat, x_hat):
    ...
    def residual(z, parameters):
        # Unpack parameters
        ode_args_fixed, inlet_bc, Ma_target = parameters
        ...
        return jnp.concatenate([R_u, R_d, R_p, jnp.asarray([R_star])])
...
if __name__ == "__main__":
    ...
    params = tree_map(
        jnp.asarray,
        {
            ...
            "x_star0":  1.00,      # initial guess for x*
        },
    )
    ...
    Ma_array = jnp.asarray(jnp.linspace(0.05, 0.15, 4))
    ...
    print("-" * 60)
    N = 50
    z0 = initialize_flowfield(jnp.linspace(0, 1, N+1), params, fluid)
