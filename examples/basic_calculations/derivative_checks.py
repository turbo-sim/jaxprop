...
st = fluid.get_props(jxp.HmassP_INPUTS, test_h, test_P)
...
dh_dT_p = jax.grad(lambda T: fluid.get_props(jxp.PT_INPUTS, st.p, T).h)(st.T)
f1 = dh_dT_p
...
de_dT_rho = jax.grad(lambda T: fluid.get_props(jxp.DmassT_INPUTS, st.d, T).e)(st.T)
f1 = de_dT_rho
...
drho_dp_T = jax.grad(lambda p: fluid.get_props(jxp.PT_INPUTS, p, st.T).rho)(st.p)
...
drho_dT_p = jax.grad(lambda T: fluid.get_props(jxp.PT_INPUTS, st.p, T).rho)(st.T)
...
dp_drho_s = jax.grad(lambda d: fluid.get_props(jxp.DmassSmass_INPUTS, d, st.s).p)(st.d)
...
dp_de_rho = jax.grad(lambda e: fluid.get_props(jxp.DmassUmass_INPUTS, st.d, e).p)(st.u)
...
dT_dp_h = jax.grad(lambda p: fluid.get_props(jxp.HmassP_INPUTS, st.h, p).T)(st.p)
f1 = dT_dp_h
f2 = st.joule_thomson
rel_error = rel_err(f1, f2)
...
dh_dp_T = jax.grad(lambda p: fluid.get_props(jxp.PT_INPUTS, p, st.T).h)(st.p)
f1 = dh_dp_T
