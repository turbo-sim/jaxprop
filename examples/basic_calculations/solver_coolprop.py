...
# State calculation
fluid = cp.Fluid(name="Water", backend="HEOS")
state = fluid.get_state(cp.PT_INPUTS, 101325, 300)
print(f"Water density is {state.rho:0.2f} kg/m3 at p={state.p:0.2f} Pa and T={state.T:0.2f} K")

# State calculation
fluid = cp.Fluid(name="Air", backend="HEOS")
state = fluid.get_state(cp.PT_INPUTS, 101325, 300)
print(f"Air heat capacity ratio is {state.gamma:0.2f} at p={state.p:0.2f} Pa and T={state.T:0.2f} K")

# Properties of liquid water
fluid = cp.Fluid("Water", backend="HEOS")
props_stable = fluid.get_state(cp.PT_INPUTS, 101325, 300)
print("\nProperties of liquid water")
print(props_stable)

# Properties of water/steam mixture
fluid = cp.Fluid("Water", backend="HEOS")
props = fluid.get_state(cp.QT_INPUTS, 0.5, 300)
print("\nProperties of water/steam mixture")
print(props)
