...
P0 = 101_325.0  # Pa
T0 = 300.0      # K
state_PT = pg.get_props_perfect_gas(cpx.PT_INPUTS, P0, T0, constants)
...
H = state_PT["h"]
S = state_PT["s"]
state_hs = pg.get_props_perfect_gas(cpx.HmassSmass_INPUTS, H, S, constants)
print("\nState from HmassSmass (h, s)")
print(state_hs)

# Property calculation using (h, p)
state_hP = pg.get_props_perfect_gas(cpx.HmassP_INPUTS, H, P0, constants)
print("\nState from HmassP (h, p)")
print(state_hP)

# Property calculation using (p, s)
state_Ps = pg.get_props_perfect_gas(cpx.PSmass_INPUTS, P0, S, constants)
print("\nState from PSmass (p, s)")
print(state_Ps)

# Property calculation using (rho, h)
rho0 = state_PT["rho"]
state_rhoh = pg.get_props_perfect_gas(cpx.DmassHmass_INPUTS, rho0, H, constants)
