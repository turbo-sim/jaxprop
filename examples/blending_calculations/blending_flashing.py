...
if PROPERTY_API == "high_level":
    props_in = fluid.get_state(cpx.PQ_INPUTS, p_in, 0)
    props_in = fluid.get_state(cpx.PT_INPUTS, p_in, props_in["T"] - dT_subcooling)
    ...
    props_in = cpx.compute_properties_coolprop(
        fluid.abstract_state, cpx.PT_INPUTS, p_in, props_in["T"] - dT_subcooling
    )
else:
